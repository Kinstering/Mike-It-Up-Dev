...
async def check_DB_exists():
    conn = sqlite3.connect('data/basic_15m_new.db')
    c = conn.cursor()
    c.execute('''
...
async def send_report(df_all, now_time):
    ...
    df_all.to_csv("logs/df_all_15m_" + str(now_time) + ".csv", sep=',')
    print('--------------------')
    ...
    await send(msg=Mtext, chat_id=-4142610422)
...
async def main():
    ...
    while True:
        ...
        if was_time != now_time:
            if os.path.isfile("data/liq_15m.csv") == True:
                shutil.copyfile('data/liq_15m.csv', 'data/liq_15m_copy.csv')
            try:
                os.remove('data/liq_15m.csv')
            except:
                print('No file: liq_15m.csv')

            # copy all data to new files
            for i in range(1,8):
              n1 = "trades_15m_"+str(i)
              n2 = "trades_15m_"+str(i)+"_copy"
            ...
            if os.path.exists('data/clusters_15m_max.csv')==False:   all_files_present = False
            if os.path.exists('data/liq_15m_copy.csv')==False:       all_files_present = False
            for i in range(1,7):
              if os.path.exists('data/trades_15m_'+str(i)+'_copy.csv')==False: all_files_present = False
              if os.path.exists('data/asks_15m_'+str(i)+'_copy.csv')==False:   all_files_present = False
              if os.path.exists('data/bids_15m_'+str(i)+'_copy.csv')==False:   all_files_present = False
            ...
            df_liq = pd.read_csv('data/liq_15m_copy.csv', sep=',', index_col=False)
            ...
            df_trades_max  = pd.read_csv("data/clusters_15m_max.csv", sep=',')
            ...
            for i in range(1,8):
                ...
                cluster_v = {}
                cluster_p = {}
                df_trades = pd.read_csv('data/trades_15m_'+str(i)+'_copy.csv', sep=',')
                ...
                df_trades_clusters.to_csv("logs/df_trades_clusters_15m_" + str(i) + ".csv", sep=',')
                # filter asks data by 4% and 8% near max cluster
                df_asks = pd.read_csv('data/asks_15m_' + str(i) + '_copy.csv', sep=',',index_col=False)
                df_asks.to_csv("logs/asks_15m_" + str(i) + "_orig.csv", sep=',')
                ...
                df_bids = pd.read_csv('data/bids_15m_'+str(i)+'_copy.csv', sep=',',index_col=False)
                df_bids.to_csv("logs/bids_15m_" + str(i) + "_orig.csv", sep=',')
                ...
                df_bids.to_csv("logs/bids_15m_" + str(i) + "_copy_v.csv", sep=',')
                ...
                df_bids.to_csv("logs/bids_15m_"+str(i)+"_copy.csv", sep=',')
            ...
            os.remove('data/liq_15m_copy.csv')
            for i in range(1,8):
              n2 = "trades_15m_"+str(i)+"_copy"
