...
def on_message(ws, message):
    ...
    if os.path.exists('data/trades_1.csv') == False:     total_df     = total_df[0:0]
    if os.path.exists('data/trades_15m_1.csv') == False: total_df_15m = total_df_15m[0:0]
    if os.path.exists('data/trades_4h_1.csv') == False:  total_df_4h  = total_df_4h[0:0]
    ...
    if was_time != now_time:
        was_time = now_time
        total_df.to_csv("data/trades_1.csv", sep=',', mode='a', header=not os.path.exists("data/trades_1.csv"))
        total_df_15m.to_csv("data/trades_15m_1.csv", sep=',', mode='a', header=not os.path.exists("data/trades_15m_1.csv"))
        total_df_4h.to_csv("data/trades_4h_1.csv", sep=',', mode='a', header=not os.path.exists("data/trades_4h_1.csv"))
